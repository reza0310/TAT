...
async function treat_result(request: XMLHttpRequest): Promise<void> {
	...
	if (out == "ACCOUNT CREATION SUCCESS") {
		...
		var req: XMLHttpRequest = new (r.request as any)("GET", u.WEBSITE_WEBPATH+"/create/success.html", {});
	} else {
		console.log(out);
		var req: XMLHttpRequest = new (r.request as any)("GET", u.WEBSITE_WEBPATH+"/create/error.html", {});
	}
...
window.onload = (): void => {
	(async () => {
		await r.request_form("POST", u.API_WEBPATH+"/register", document.getElementById("form")! as HTMLFormElement, treat_result);
	})();
}
